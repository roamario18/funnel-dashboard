  ...
  return Promise.all(
    codes.map((code) =>
      aggregateStagesForCurrency(filter, code, authorization, field)
        .then((json) => ({ code, json, ok: true, error: null }))
        .catch((err) => ({ code, json: null, ok: false, error: summarizeAggError(err) })),
    ),
  );
}

function pickCurrencyCodes(directoryCodes, fromRecent) {
  const directory = [...new Set((directoryCodes || []).map(currencyCode).filter(Boolean))];
  const recent = [...new Set((fromRecent || []).map(currencyCode).filter(Boolean))];
  const inBoth = recent.filter((code) => directory.includes(code));
  if (inBoth.length) return inBoth;
  if (directory.length) return directory.slice(0, 3);
...
async function loadCurrencyAggregates(filter, codes, authorization) {
  let results = await runCurrencyAggregates(filter, codes, authorization, 'currency');
  if (results.some((item) => item.ok)) return results;
  if (results.every((item) => isTransientAgg(item.error))) throwAggregateFailed(results);

  const probeCode = codes[0];
  ...
  if (probe.ok) {
    results = await runCurrencyAggregates(filter, codes, authorization, 'currencyId');
    if (results.some((item) => item.ok)) return results;
  } else {
...
async function loadDashboard(query, authorization) {
  ...
  const directoryCodes = [...new Set((currencyPack.codes || []).map(currencyCode).filter(Boolean))];
  const currencyCodes = pickCurrencyCodes(directoryCodes, fromRecent);
  const [aggregates, users] = await Promise.all([
    loadCurrencyAggregates(filter, currencyCodes, authorization),
    usersPromise,
  ]);
  ...
  const failedAggregates = aggregates.filter((item) => !item.ok);
  let totalRub = 0;
  let totalCount = 0;
  let openAmount = 0;
  let wonCount = 0;
  ...
  } else if (directoryCodes.length > currencyCodes.length) {
    warnings.push(
      `В справочнике ${directoryCodes.join(', ')} — агрегация по валютам из последних сделок (${currencyCodes.join(', ')}).`,
    );
  }
  if (truncated) {

...
async function loadDashboard(query, authorization) {
  ...
  let currencies = [...new Set(recentDeals.map((d) => d.currency).filter(Boolean))];
  try {
    const currencyJson = await vibeFetch('/v1/deals/aggregate', {
      method: 'POST',
      authorization,
      body: {
    ...
    const groupsCur = Array.isArray(currencyJson.data?.groups) ? currencyJson.data.groups : [];
    const fromAgg = groupsCur
      .map((g) => String(g.currency || g.currencyId || g.stageId || g.groupValue || ''))
      .filter((v) => v && v !== 'undefined');
    if (fromAgg.length) currencies = [...new Set(fromAgg)];
  } catch {
    // groupBy currency может быть недоступен — опираемся на последние сделки
  }
  if (currencies.length > 1) {
    warnings.push(
      `В выборке смешаны валюты (${currencies.join(', ')}). Суммы KPI и стадий не приведены к одной валюте.`,
    );
  }

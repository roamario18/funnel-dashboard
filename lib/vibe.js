...
async function vibeFetch(path, { method = 'GET', body, authorization, timeoutMs = 8000, maxAttempts = 1 } = {}) {
  const key = apiKey();
  if (!key) {
  ...
  while (true) {
    ...
    try {
      ...
      clearTimeout(timer);
      const aborted = Boolean(cause) && (cause.name === 'AbortError' || cause.code === 'ABORT_ERR');
      if (!aborted && attempt < maxAttempts) {
        await sleep(1000 * 2 ** (attempt - 1));
        continue;
      }
      const err = new Error(
        aborted ? 'CRM не успел ответить за отведённое время' : 'VibeCode API недоступен',
      );
      err.code = aborted ? 'CRM_TIMEOUT' : 'BITRIX_UNAVAILABLE';
    ...
    if (res.status === 429 && attempt < maxAttempts) {
      ...
      const waitMs = (Number.isFinite(waitSec) ? Math.min(Math.max(waitSec, 0), 2) : 1) * 1000;
      await sleep(waitMs);
      continue;
    }

    if (!res.ok || json.success === false) {
      const code = json.error?.code || (res.status === 429 ? 'RATE_LIMITED' : `HTTP_${res.status}`);
